/**
 * dsh-proxy — host-side settings wiring.
 *
 * Two halves:
 *
 ...
 *    (`/api/dsh-proxy/settings/{describe,mutate}`) riding the host
...
// No import from '@deepseek-ai/dsh-settings' on purpose: the settings SEAM
// instance is injected by the host (its own dsh-settings copy), while this
// plugin would resolve a DIFFERENT copy from its own node_modules — the two
// module graphs share no class identity. 0.1.2-rc.1 also stopped exporting
// `settingsNamespace`. Everything the plugin needs from the package is
// reproduced locally with the exact host semantics:
const NAMESPACE_PATTERN = /^[a-z][a-z0-9-]*$/

/** Validate a namespace id like the host's parseSettingsNamespace (same pattern). */
...
export const LLM_PROXY_NAMESPACE = settingsNamespace('llm-proxy')

/** Bridge route prefix (same-origin, loopback-only). */
export const SETTINGS_BRIDGE_PREFIX = '/api/dsh-proxy/settings'
...
function failureOf(error) {
  // Structural (not instanceof on the CLASS) on purpose: the seam throws the
  // HOST copy's SettingsConflictError, whose class identity differs from any
  // copy this plugin could resolve — cross-copy instanceof is always false.
  // Both 0.1.2-rc.1 and 0.1.0-rc.8 set `this.name = "SettingsConflictError"`
  // in the constructor and carry the stable `code = "SETTINGS_CONFLICT"`
  // class field; matching either marker (absorbed from cross-audit) keeps
  // working if a future version ever changes just one of the two.
